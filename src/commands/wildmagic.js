...
import {
    ...
    TASHAS_SURGE_TABLE,
    SURGE_TRIGGERS
} from '../data/wild-magic.js';

// Pure function to create initial state
const createWildMagicState = () => ({
    surgeCount: 0,
    spellsCast: 0,
    currentThreshold: 1,
...
// Pure function to check for surge
const checkForSurge = (state, spellLevel = 1) => {
    const roll = rollD20();
...
// Pure function to get surge effect
const getSurgeEffect = state => {
    const roll = rollD100();
...
// Functions for custom table management
const loadCustomTable = async (state, tablePath) => {
    try {
        const tableData = await fs.readJson(tablePath);
        const newCustomTables = new Map(state.customTables);
        newCustomTables.set(tableData.name, tableData.effects);
        ...
        console.error('Error loading custom table:', error);
        return {
            success: false,
            newState: state
        };
...
const displayStats = state => {
    ...
    console.log(chalk.green(`Surge Rate: ${((state.surgeCount / state.spellsCast) * 100 || 0).toFixed(1)}%`));
    console.log(chalk.green(`Current Trigger Method: ${SURGE_TRIGGERS[state.triggerType].name}`));
        ...
        switch (action) {
            case 'CAST': {
                ...
                if (spellLevel === 0 && state.triggerType !== 'SPELL_LEVEL') {
                    console.log(chalk.yellow("Cantrips don't trigger Wild Magic Surges."));
                    continue;
                }
            ...
            case 'TRIGGER': {
                ...
                console.log(chalk.green(`Surge trigger changed to: ${SURGE_TRIGGERS[trigger].name}`));
                break;
            }
            ...
            case 'TABLE': {
                const { table } = await inquirer.prompt({
                    ...
                    choices: [
                        ...
                        { name: "Tasha's Cauldron of Everything", value: TASHAS_SURGE_TABLE },
                        ...Array.from(state.customTables.entries()).map(([name, table]) => ({
                            name: `Custom: ${name}`,
                            value: table
                        }))
                    ]
            ...
            case 'CREATE': {
                ...
                const { success, newState } = await saveCustomTable(state, tableName, effects);
                state = newState;
                if (success) {
