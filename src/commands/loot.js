...
export const rollTreasure = (cr) => {
    ...
    Object.entries(result.coins).forEach(([type, diceString]) => {
        treasure[type.replace('cp', 'copper')
                   .replace('sp', 'silver')
                   .replace('ep', 'electrum')
                   .replace('gp', 'gold')
                   .replace('pp', 'platinum')] = parseTreasureRoll(diceString);
    });
...
export const generateMagicItems = (table, count) => {
    ...
    for (let i = 0; i < count; i++) {
        const roll = Math.floor(Math.random() * 100) + 1;
        items.push(rollOnTable(MAGIC_ITEM_TABLES[table], roll).item);
    }
    return items;
};
...
export const generateHoard = (cr) => {
    ...
    if (gemsArtResult.items) {
        gemsArtResult.items.forEach(item => {
    ...
    // Roll for magic items if any
    const magicItems = [];
    if (gemsArtResult.magic_items) {
        gemsArtResult.magic_items.forEach(item => {
            const count = parseTreasureRoll(item.count);
            magicItems.push(...generateMagicItems(item.table, count));
        });
    }

    const totalValue = calculateHoardValue(coins) +
...
export const generateLoot = async () => {
    const answers = await inquirer.prompt([
        ...
        },
        {
            ...
            validate: input => {
                ...
                return !isNaN(num) && num >= 0 && num <= 30 || 'Please enter a number between 0 and 30';
            },
    ...
    results.forEach((loot, index) => {
        ...
        if (answers.treasureType === 'individual') {
            console.log(chalk.green('\nIndividual Treasure:'));
            if (loot.coins.copper) console.log(chalk.yellow('Copper:'), loot.coins.copper);
            if (loot.coins.silver) console.log(chalk.white('Silver:'), loot.coins.silver);
            if (loot.coins.electrum) console.log(chalk.blue('Electrum:'), loot.coins.electrum);
            if (loot.coins.gold) console.log(chalk.yellow('Gold:'), loot.coins.gold);
            if (loot.coins.platinum) console.log(chalk.cyan('Platinum:'), loot.coins.platinum);
            ...
            if (loot.valuables.length > 0) {
                ...
                loot.valuables.forEach(item => {
                    console.log(`${chalk.blue(item.type === 'gem' ? 'Gem' : 'Art Object')} (${item.value} gp):`, item.description);
                });
            }
    ...
    if (saveFormat !== 'none') {
        ...
        if (saveFormat === 'txt' || saveFormat === 'both') {
            const txtFilepath = path.join(process.cwd(), 'loot', `${baseFilename}.txt`);
            const txtContent = results.map((loot, index) => {
                if (answers.treasureType === 'individual') {
                    return `
...
`;
        ...
        if (saveFormat === 'json' || saveFormat === 'both') {
            const jsonFilepath = path.join(process.cwd(), 'loot', `${baseFilename}.json`);
            const jsonContent = {
                metadata: {
                    ...
                    customTable: answers.useCustomTable ? answers.customTablePath : undefined
                },
                results
            };
